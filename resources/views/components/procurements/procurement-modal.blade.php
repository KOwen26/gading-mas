                                    ...
                                    @if ($companies)
                                        @foreach ($companies as $company)
                                            <option value="{{ $company->company_id }}"
                                                @if ($procurement?->company_id == $company->company_id) selected @endif>
                                    ...
                                    @if ($suppliers)
                                        @foreach ($suppliers as $supplier)
                                            <option value="{{ $supplier->supplier_id }}"
                                                @if ($procurement?->supplier_id == $supplier->supplier_id) selected @endif>
                                                            ...
                                                            @if ($products)
                                                                ...
                                                                @foreach ($products as $product)
                                                                    <option value="{{ $product->product_id }}"
                                                                        @if ($procurement_detail?->product_id == $product->product_id) selected @endif
                                        ...
                                        <tr>
                                            <td colspan="5">
                                                <button type="button" id="add_row"
                                                    @if ($procurement?->procurementDetails) onclick="procurementAddRowUpdate(this)"
                                                    @else
                                                    onclick="procurementAddRow(this)" @endif
                            ...
                            <div class="row">
                                <div class="col-md-3">
                                    <div class="form-group"><label for="" class="text-bold">Diskon
                                        ...
                                        <input type="number" min="0"
                                            ...
                                            class="form-control" name="procurement_discount"
                                            id="procurement_discount"
                                            value="{{ $procurement?->procurement_discount && (int)$procurement?->procurement_total > 0 ? round(($procurement?->procurement_discount / $procurement?->procurement_total) * 100, 2) : 0 }}">
                                    </div>
                                </div>
                                <div class="col-md-3">
                                        ...
                                        <select class="form-control" name="payment_status" id="payment_status">
                                            @foreach ($payment_status as $status)
                                                <option value="{{ $status[0] }}"
                                                    @if ($procurement?->payment_status == $status[0]) selected @endif>
                                                    {{ $status[1] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group"><label for="" class="text-bold">Status
                                        ...
                                        <select class="form-control" name="procurement_status"
                                            ...
                                            @foreach ($statuses as $status)
                                                <option value="{{ $status[0] }}"
                                                    @if ($procurement?->procurement_status == $status[0]) selected @endif>
                                                    {{ $status[1] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        @if ($procurement?->ProcurementDetails)
                                            <div class="form-check">
                                                <input class="form-check-input" onchange="calculateGrandTotalUpdate()"
                                                    type="checkbox" name="procurement_tax"
                                                    id="procurement_tax_check_update" value="true" value="true"
                                                    @if ($procurement?->procurement_tax > 0) checked @endif>
                                    ...
                                    <div class="form-group"><label for="" class="text-bold">Export
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input" onchange="" type="checkbox"
                                                ...
                                                @if ($procurement?->procurement_audit == 'true') checked @endif>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group"><label for="" class="text-bold">Catatan
                                            Pesanan</label>
                                        <textarea class="form-control" name="procurement_notes" id="procurement_notes" cols="30" rows="2">{{ $procurement?->procurement_notes }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class=" float-end">
                                        <input class="form-control-plaintext form-control-sm text-end " readonly
                                            type="number" name="" id="procurement_total_update"
                                            value="{{ intval($procurement?->procurement_total) ?? 0 }}"
                                            autocomplete="off">
                                        <div class="inline">
                                        </div>
                                    </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="float-end">
                                        <input class="form-control-plaintext form-control-sm text-danger text-end"
                                            readonly type="number" name="" id="procurement_discount2_update"
                                            value="{{ intval($procurement?->procurement_discount) ?? 0 }}">
                                    </div>
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    <span>PPN (11%) :</span>
                                    <div class="float-end">
                                        <input class="form-control-plaintext form-control-sm text-end" readonly
                                            type="number" name="" id="procurement_tax_update"
                                            value="{{ intval($procurement?->procurement_tax) ?? 0 }}">
                                    </div>
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="float-end">
                                        <input class="form-control-plaintext form-control-sm text-end" readonly
                                            type="number" name="" id="procurement_grand_total_update"
                                            value="{{ intval($procurement?->procurement_grand_total) ?? 0 }}">
                                    </div>
                                </div>
                            @else
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class=" float-end">
                                        <input class="form-control-plaintext form-control-sm text-end " readonly
                                            type="number" name="" id="procurement_total"
                                            value="{{ intval($procurement?->procurement_total) ?? 0 }}"
                                            autocomplete="off">
                                        <div class="inline">
                                        </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="float-end">
                                        <input class="form-control-plaintext form-control-sm text-danger text-end"
                                            readonly type="number" name="" id="procurement_discount2"
                                            value="{{ intval($procurement?->procurement_discount) ?? 0 }}">
                                    </div>
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="float-end">
                                        <input class="form-control-plaintext form-control-sm text-end" readonly
                                            type="number" name="" id="procurement_tax"
                                            value="{{ intval($procurement?->procurement_tax) ?? 0 }}">
                                    </div>
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="float-end">
                                        <input class="form-control-plaintext form-control-sm text-end" readonly
                                            type="number" name="" id="procurement_grand_total"
                                            value="{{ intval($procurement?->procurement_grand_total) ?? 0 }}">
                                    </div>
                                </div>
                            @endif
...
@push('script')
    <script defer>
        ...
        function calculateGrandTotalUpdate() {
            ...
            grand_total = total - discount
            // console.log({
            //     procurement_tax_check_update,
            //     procurement_total_label_update,
            //     procurement_discount_label_update,
            //     procurement_tax_label_update,
            //     procurement_grand_total_label_update
            // })
        ...
        function procurementAddRow(elem) {
            ...
            if (index < procurementMaxRow) {
                $('#dynamic-procurement-product').append(
        ...
        function calculate() {
            ...
            while (count != index) {
                ...
                product_price = document.querySelector(`[id="product_price"][index="${count}"]`) && document.querySelector(
                    `[id="product_price"][index="${count}"]`).value;
        ...
        function calculateUpdate() {
            ...
            while (count != index) {
                ...
                product_price = document.querySelector(`[id="product_price_update"][index="${count}"]`) && parseInt(document
                    .querySelector(`[id="product_price_update"][index="${count}"]`).value);
                product_subtotal = product_quantity * product_price
