            ...
            @if ($products->count() > 0)
                @foreach ($products as $product)
                    <tr>
                        ...
                        <td data-order="{{ intval($product?->product_quantity) }}">
                            {{ $product->product_quantity ?? 0 }} {{ $product->product_unit ?: 'pcs' }}</td>
                        @if (auth()->user()->user_type === 'OWNER')
                            <td data-order="{{ intval($product?->product_buy_price) }}">Rp
                                {{ number_format($product->product_buy_price ?? 0, '0', '0', '.') }}</td>
                        @endif
                            ...
                            <div class="d-flex gap-2">
                                ...
                                @if (auth()->user()->user_type === 'OWNER')
                                    <button type="button" class="btn btn-danger" onclick="Delete(this)"
                                        ...
                                        product_name="{{ $product->product_name }}" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="Hapus Produk"><i
...
@push('script')
    <script>
        ...
        function ProductMovement(elem) {
            let url = "{{ route('product.movement', [':id']) }}"
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $("#modal-detail").find("#product-movement").modal('show');
                    let table2 = document.querySelector('#table-movement');
        ...
        function Delete(elem) {
            ...
            let product_id = elem.getAttribute("product_id")
            let product_name = elem.getAttribute("product_name")
